...
fn propagate_user_books(mut commands: Commands) {
    commands.spawn(Book {name: "Słowa światłości".to_string(), author: "Brandon Sanderson".to_string()});
    commands.spawn(Book {name: "Elantris".to_string(), author: "Brandon Sanderson".to_string()});
    commands.spawn(Book {name: "Red Rising".to_string(), author: "Pierce Brown".to_string()});
}
...
impl Plugin for UserBooksPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(UserBooksTimer(Timer::from_seconds(2.0, TimerMode::Repeating)))
